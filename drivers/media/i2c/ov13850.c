...
static long ov13850_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    case RKMODULE_GET_MODULE_INFO:
        if (ov13850->module_name) {
            dev_err(dev, "ov13850_get_module_inf: %s\n",ov13850->module_name);
            ov13850_get_module_inf(ov13850, (struct rkmodule_inf *)arg);
        }
        else {
            dev_err(dev, "No camaera module_name!\n");
            ret = -ENOIOCTLCMD;
        }
        break;
    default:
             ...
             const struct i2c_device_id *id)
{
    ...
    if (ret) {
        dev_err(dev, "ret: %d - could not get '%s'\n", ret, RKMODULE_CAMERA_MODULE_INDEX);
    }
    ret |= of_property_read_string(node, RKMODULE_CAMERA_MODULE_FACING,
                       &ov13850->module_facing);
    if (ov13850->module_facing)
        dev_err(dev, "module_facing: %s\n",ov13850->module_facing);
    if (ret) {
        dev_err(dev, "ret: %d - could not get '%s'\n", ret, RKMODULE_CAMERA_MODULE_FACING);
    }
    ...
    if (ret) {
        dev_err(dev, "ret: %d - could not get '%s'\n", ret, RKMODULE_CAMERA_MODULE_NAME);
    }
    ret |= of_property_read_string(node, RKMODULE_CAMERA_LENS_NAME,
                       &ov13850->len_name);
    if (ret) {
        dev_err(dev, "ret: %d - could not get '%s'\n", ret, RKMODULE_CAMERA_LENS_NAME);
        // return -EINVAL;
    }
